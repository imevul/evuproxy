...
func nftablesRoutes(c *config.Config) (string, error) {
	...
	type formattedRoute struct {
		...
		portExpr string
		target   string
		srcSet   string // nft set name without @; empty = allow any WAN source
	}
	...
	for i, r := range c.Forwarding.Routes {
		...
		if normalized := normalizedRouteSourceElems(r.SourceAllowCIDRs); len(normalized) > 0 {
			srcSet = routeSrcSetName(i)
		}
		for _, p := range protos {
			routes = append(routes, formattedRoute{p, expr, target, srcSet})
...
`)
	...
	for _, r := range routes {
		writeGeoProtoPortInputRules(&b, c.Geo.Enabled, blockListed, geoSet, r.proto, r.portExpr, r.srcSet)
	}
...
`)
	...
	for _, r := range routes {
		writeDnatRule(&b, c.Geo.Enabled, blockListed, geoSet, r.proto, r.portExpr, r.target, r.srcSet)
	}
