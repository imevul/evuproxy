...
Turnkey **TCP/UDP exposure** on a Linux VPS using **WireGuard** and **nftables**, with a declarative config and optional **GeoIP** country filtering (**allow** or **block** mode in config). The supported interface is **this repository’s YAML config + `evuproxy` CLI/API** (and optional Docker admin UI).
