...
| Entrypoint stuck / `i/o timeout` to `version.crowdsec.net` or `api.crowdsec.net` | Outbound HTTPS to CrowdSec is blocked. Compose sets `DISABLE_ONLINE_API` and `NO_HUB_UPGRADE` so LAPI starts offline (local bans/scenarios only). `docker compose up -d crowdsec` and wait ~90s; `ps aux` should show `crowdsec`, not only `docker_start.sh`. |
